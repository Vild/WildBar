...
xcb_set_bg (int i)
{
  xcb_change_gc (c, draw_gc  , XCB_GC_BACKGROUND, (const unsigned [])
  {
    palette[i]
  });
  xcb_change_gc (c, clear_gc , XCB_GC_FOREGROUND, (const unsigned [])
  {
    palette[i]
  });
}
...
xcb_set_fg (int i)
{
  xcb_change_gc (c, draw_gc , XCB_GC_FOREGROUND, (const uint32_t [])
  {
    palette[i]
  });
}
...
xcb_set_ud (int i)
{
  xcb_change_gc (c, underl_gc, XCB_GC_FOREGROUND, (const uint32_t [])
  {
    palette[i]
  });
}
...
{
  xcb_poly_fill_rectangle (c, canvas, gc, 1, (const xcb_rectangle_t [])
  {
    {
      x, y, width, height
    }
...
xcb_set_fontset (int i)
{
  ...
  {
    sel_font = &fontset[i];
    xcb_change_gc (c, draw_gc , XCB_GC_FONT, (const uint32_t [])
    {
      sel_font->xcb_ft
    });
  }
...
static inline void
xcb_handle_event (screen_t * screen, int x)
{
...
set_ewmh_atoms (void)
{
  ...
  for (screen_t * cur_screen = screens; cur_screen < screens+num_screens; cur_screen++)
  {
    ...
    xcb_change_property (c, XCB_PROP_MODE_REPLACE, cur_screen->window, atom_list[NET_WM_WINDOW_OPACITY], XCB_ATOM_CARDINAL, 32, 1, (const uint32_t [])
    {
      (uint32_t)(bar_opacity * 0xffffffff)
    } );
    ...
    xcb_change_property (c, XCB_PROP_MODE_REPLACE, cur_screen->window, atom_list[NET_WM_DESKTOP], XCB_ATOM_CARDINAL, 32, 1, (const uint32_t [])
    {
      -1
    } );
...
create_window(xcb_window_t root, int x, int y, int width, int height, xcb_visualid_t visual)
{
  ...
  xcb_create_window(c, XCB_COPY_FROM_PARENT, window, root, x, y, width, height, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, visual, XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK, (const uint32_t [])
  {
    palette[10], XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_BUTTON_RELEASE
  });

  xcb_change_window_attributes (c, window, XCB_CW_OVERRIDE_REDIRECT, (const uint32_t [])
  {
    force_docking
  });
...
init (void)
{
  ...
  xcb_create_gc (c, draw_gc, root, XCB_GC_FOREGROUND | XCB_GC_BACKGROUND, (const uint32_t [])
  {
    palette[11], palette[10]
  });

  clear_gc = xcb_generate_id (c);
  xcb_create_gc (c, clear_gc, root, XCB_GC_FOREGROUND, (const uint32_t [])
  {
    palette[10]
  });

  underl_gc = xcb_generate_id (c);
  xcb_create_gc (c, underl_gc, root, XCB_GC_FOREGROUND, (const uint32_t [])
  {
    palette[10]
  });
...
bar_main ()
{
  ...
  while (!quit)
  {
    ...
    {
      if(pollin[0].revents & POLLIN)   /* Xserver broadcasted an event */
      {
        while((ev = xcb_poll_for_event(c)))
        {
          ...
          case XCB_BUTTON_RELEASE:
            ...
            if(button_ev->detail == mouse_button)
            {
              ...
              for (screen_t * cur_screen = screens; cur_screen < screens + num_screens; cur_screen++)
                if (cur_screen->window == button_ev->event)
                {
                  xcb_handle_event(cur_screen, button_ev->event_x);
                  break;
                }
            }
